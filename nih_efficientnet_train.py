...
torch.multiprocessing.set_sharing_strategy('file_system')   # dgxhnode5 has a small /dev/shm
...
NUM_WORKERS  = 4      # dgxhnode5: 4 + file_system sharing is the proven-stable combo
NUM_EPOCHS   = 18     # FAST: high LR converges by ~ep12 (was 60, conservative-LR crawl)
IMG_SIZE     = 380    # ← EfficientNet-B4 native resolution
LR_HEAD      = 2e-4   # FAST convergence (was 3e-5) — same recipe your CheXpert runs used
LR_BACKBONE  = 6e-5   # FAST convergence (was 1e-5)
...
MIN_EPOCHS_STOP    = 8        # allow early stop sooner (was 30)
...
AUTO_RESUME        = False    # start fresh with the fast recipe (crashes are fixed now)
...
class ChestXrayDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        path = row["full_path"]
        try:
            img = Image.open(path).convert("RGB")   # 3-channel: keep full pretrained stem
...
class GAPGMP(nn.Module):
    # Paper 4 (NIH ChestX-ray14): GAP + Global-Max-Pool, ablation-proven +1.0% AUC.
    # GAP captures diffuse findings (Edema/Infiltration); GMP captures focal ones
    # (Nodule/Mass/Pneumothorax). Element-wise sum keeps the 1792-dim vector unchanged.
    def __init__(self):
        super().__init__()
...
def create_efficientnet(num_classes=14):
    ...
    # 3-channel input: keep the FULL ImageNet-pretrained RGB stem (grayscale is
    # replicated to 3 channels in the dataset). Preserves more transfer learning
    # than averaging the stem down to 1 channel.
    ...
    model.avgpool = GAPGMP()   # Paper 4: dual GAP+GMP pooling (+1% AUC, 0 params)

    total = sum(p.numel() for p in model.parameters())
    print(f"✅ EfficientNet-B4 ready — {total/1e6:.1f}M parameters (GAP+GMP dual pooling)")
...
def get_train_transform():
    return T.Compose([
        ...
        T.RandomPerspective(distortion_scale=0.1, p=0.3),   # Paper 8: Cardiomegaly angle robustness
        ...
        T.ToTensor(),
        T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),  # ImageNet 3-ch
...
def train_loop(model, train_df, val_loader, device, optimizer, scheduler,
    ...
    train_loader = DataLoader(
        ...
        persistent_workers=False   # release workers each epoch — prevents shm accumulation
    )
...
def main():
    ...
    val_loader = DataLoader(
        ...
        shuffle=False,
        num_workers=0,               # single-process val — bulletproof, zero worker bus errors
        pin_memory=(device.type == "cuda"),
    )
